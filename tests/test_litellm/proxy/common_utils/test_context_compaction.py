...
@pytest.fixture
def summary_proxy(monkeypatch: pytest.MonkeyPatch) -> Generator[_PROXY_MaxParallelRequestsHandler_v3, None, None]:
...
@pytest.mark.asyncio
@pytest.mark.parametrize("outcome", ["success", "denied", "cancel", "timeout"])
async def test_real_proxy_summary_auth_identity_and_lifecycle(
    outcome: Literal["success", "denied", "cancel", "timeout"],
    summary_proxy: _PROXY_MaxParallelRequestsHandler_v3, monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    await summary_proxy.async_pre_call_hook(auth, DualCache(), parent_data, "acompletion")
    ...
    async def receive(**provider_data: object) -> ModelResponse:
        ...
        for name, expected in (
            ...
            ("litellm_session_id", "session"), ("messages", _MESSAGES),
        ):
        ...
        try:
            ...
            return ModelResponse(choices=[{"message": {"role": "assistant", "content": "Summary"}}],
                                 usage=Usage(prompt_tokens=10, completion_tokens=2, total_tokens=12))
        finally:
            stopped.set()

    inference: Final = AsyncMock(side_effect=receive)
    monkeypatch.setattr(litellm, "acompletion", inference)
    assert current_summary_executor() is None
    with proxy_summary_executor_scope(parent_request(application), parent_data, summary_proxy):
        executor: Final = current_summary_executor()
        assert executor is not None
        task: Final = asyncio.create_task(executor("summary", _MESSAGES, 128, 0.2 if outcome == "timeout" else 10))
        if outcome == "denied":
            ...
            assert cached.models == ["target"] and cached.spend == 0
        else:
            if outcome == "success":
                assert (await task).choices[0].message.content == "Summary"
            else:
    ...
    with pytest.raises(ValueError, match="closed"):
        await executor("summary", _MESSAGES, 128, 10)
    await summary_proxy.async_post_call_success_hook(parent_data, auth, ModelResponse())
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=proxy_server.app)) as client:
        rejected: Final = await client.post(
            "https://gateway.test/v1/chat/completions", headers={"authorization": f"Bearer {_KEY}"},
